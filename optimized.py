...
class Iko:
    ...
    def speak(self, text):
        ...
        playing.set()
        CHUNK_SAMPLES = 2048  # ~128ms at 16kHz, good balance for Pi
        ...
        def stream_to_queue():
            ...
            for chunk in stream:
                if chunk:
                    buffer += chunk
                    while len(buffer) >= CHUNK_SAMPLES * 2:  # *2 for int16 bytes
                        audio_queue.put(buffer[:CHUNK_SAMPLES * 2])
        ...
        def play():
            started = False
            with sd.OutputStream(samplerate=16000, channels=1, dtype='float32') as out:
        ...
        def watch_for_interruption():
            ...
            time.sleep(0.5)
            with sd.RawInputStream(samplerate=self.SAMPLE_RATE, channels=1,
                                   dtype='int16', blocksize=self.CHUNK_SIZE) as s:
                while playing.is_set():
                    chunk, _ = s.read(self.CHUNK_SIZE)
                    chunk_array = np.frombuffer(chunk, dtype=np.int16).astype(float)
                    if np.abs(chunk_array).mean() < 5000:
                        continue
